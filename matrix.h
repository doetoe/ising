...
class Matrix
{
  ...
  public:
    ...
    std::valarray<int8_t>& data() {return entries_;}
    const std::valarray<int8_t>& data() const {return entries_;}
    ...
    int8_t get(uint32_t r, uint32_t c) const { return entries_[index(r,c)]; }
    void set(uint32_t r, uint32_t c, int8_t x) { entries_[index(r,c)] = x; }
